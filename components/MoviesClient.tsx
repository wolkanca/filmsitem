...
function isTV(movie: MovieCardMovie): boolean {
...
export default function MoviesClient({ movies }: MoviesClientProps) {
  ...
  function isOther(movie: MovieCardMovie): boolean {
  ...
  // Not: plot/overview araması bundle boyutunu aşırı artırdığından devre dışıdır.
  // Arama; başlık, yönetmen, oyuncu, senarist, tür, liste ve yıl üzerinde çalışır.
  const filteredAndSortedMovies = useMemo(() => {
    let candidateMovies = [...tabFilteredMovies];
    const isSearching = Boolean(searchQuery.trim());
    const priorityMap = new Map<MovieCardMovie, number>();
    ...
    if (isSearching) {
      ...
      candidateMovies = candidateMovies.filter((movie) => {
        ...
        if (isTitleMatch || isPersonMatch || isCategoryMatch) {
    ...
    candidateMovies.sort((a, b) => {
      // Arama aktifse öncelik sıralaması
      if (isSearching) {
