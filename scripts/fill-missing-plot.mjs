...
function shouldFetchPlot(movie) {
    if (forceUpdate) return true;

    return (
        !Object.prototype.hasOwnProperty.call(movie, 'plot') ||
        movie.plot === undefined ||
        movie.plot === null ||
        String(movie.plot).trim() === '' ||
        String(movie.plot).trim().toUpperCase() === 'N/A'
    );
}
...
for (const movie of moviesToProcess) {
    ...
    if (!shouldFetchPlot(movie)) {
        skippedCount++;
        console.log(`Plot dolu, atlandı: ${movie.title} (${movie.imdbId})`);
        continue;
    }
    ...
    try {
        console.log(`API çağrısı yapılacak: ${movie.title} (${movie.imdbId})`);
