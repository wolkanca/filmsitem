...
export default async function ListsPage() {
  ...
  const smartCollections: SmartCollection[] = [
    {
      ...
      filter: (movie) => hasGenre(movie, ['action', 'adventure', 'war']),
      order: 11,
    },
    {
      name: 'Dram',
    ...
    },
    {
      ...
      filter: (movie) => hasGenre(movie, ['horror', 'thriller', 'mystery']),
      order: 13,
    },
    {
      name: 'Belgeseller',
      description: 'Documentary türündeki belgesel film, dizi ve özel yapımlar.',
      filter: (movie) => hasGenre(movie, ['documentary', 'biography']),
      order: 14,
    },
    {
      name: 'Türk Filmleri',
  ...
  Object.values(listsMap).forEach((list) => {
    list.poster = getRandomPoster(list.posters);
  });

  const smartCollectionsMap = Object.fromEntries(
    smartCollections.map((collection) => [collection.name, collection])
  );

  const lists = Object.values(listsMap)
    .filter((list) => list.count > 0)
    .sort((a, b) => {
      const orderA = listOrder[a.name] ?? smartCollectionsMap[a.name]?.order ?? 99;
      const orderB = listOrder[b.name] ?? smartCollectionsMap[b.name]?.order ?? 99;
              ...
              >
                {list.poster && (
                  <div className="absolute inset-0 z-0 opacity-15 transition-opacity duration-500 group-hover:opacity-25">
                    <Image src={list.poster} alt={list.name} fill className="object-cover" />
